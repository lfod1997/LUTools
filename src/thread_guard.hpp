...
#pragma region Move-construct-only
    ...
    ThreadGuard(ThreadGuard&& src) noexcept: _th(std::move(src._th)) { }
...
#pragma endregion
    ...
    explicit ThreadGuard(ValTy&& ...args): _th(ThreadTy { std::forward<ValTy>(args)... }) { }

    explicit ThreadGuard(ThreadTy&& th): _th(std::move(th))
    {
        if (!_th.joinable())
        {
